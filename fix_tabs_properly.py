...
"""
    ...
    if 'custom_liquid_CxGYaH' in sections or 'custom_liquid_hYpwa8' in sections:
        # Create new tabs section
        new_section_id = 'custom_liquid_tabs'
        ...
        added_tabs = False
        for s_id in order:
            if s_id in ['custom_liquid_CxGYaH', 'custom_liquid_hYpwa8']:
                if not added_tabs:
                    new_order.append(new_section_id)
                    added_tabs = True
            else:
                new_order.append(s_id)
                
        if 'custom_liquid_CxGYaH' in sections:
            del sections['custom_liquid_CxGYaH']
        if 'custom_liquid_hYpwa8' in sections:
            del sections['custom_liquid_hYpwa8']
